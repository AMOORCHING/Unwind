...
export function App() {
    ...
    const handleNavigateToRun = useCallback((runId) => {
    ...
    if (view === "detail" && selectedRun) {
        return (<RunDetail run={selectedRun} onBack={handleBack} onNavigateToRun={handleNavigateToRun}/>);
    }
    return (<RunList key={reloadKey} dbFilename={dbFilename} onReload={handleReload} onChangeFile={handleChangeFile} onSelectRun={handleSelectRun}/>);
}
