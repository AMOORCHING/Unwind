...
describe("Unwind – fork-from-checkpoint", () => {
  ...
  function makeCountedTool(name: string, counter: { count: number }) {
    return unwind.tool({
      name,
  ...
  function makeReversibleTool(
    name: string,
    counter: { count: number },
    compensateTracker: { calls: string[] }
  ) {
    return unwind.tool({
      name,
      ...
      compensate: async (args) => {
        compensateTracker.calls.push(args.id as string);
  ...
  it("compensates only the forked run's events, not the parent's", async () => {
    const counter = { count: 0 };
    const parentCompensateTracker = { calls: [] as string[] };
    const forkCompensateTracker = { calls: [] as string[] };

    const parentReversible = makeReversibleTool(
      "createRecord",
      counter,
      parentCompensateTracker
    );
    ...
    const forkReversible = makeReversibleTool(
      "createForkRecord",
      counter,
      forkCompensateTracker
    );
    ...
    expect(
      summary.compensated.some((c) => c.toolName === "createForkRecord")
    ).toBe(true);
  ...
  it("throws when forking from a step beyond completed range", async () => {
    ...
    expect(() => unwind.fork(parentRunId, { fromStep: 10 })).toThrow(
      /out of range/
    );
  });
  ...
  it("throws when forking from a nonexistent parent run", () => {
    expect(() =>
      unwind.fork("nonexistent-run-id", { fromStep: 0 })
    ).toThrow(/not found/);
  });
