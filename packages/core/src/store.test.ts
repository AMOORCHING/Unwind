...
import type { UnwindEvent, ToolCallTracked, ToolCallCompleted, RunFailed } from "./types.js";
...
function makeToolCallTracked(stepIndex: number): ToolCallTracked {
  const id = uuid();
  return {
...
function makeToolCallCompleted(toolCallId: string, stepIndex: number): ToolCallCompleted {
  return {
...
describe("SQLiteEventStore", () => {
  ...
  it("preserves event data fidelity through JSON round-trip", () => {
    ...
    const events = store.getEvents(run.id);
    const retrieved = events[0] as RunFailed;
