...
describe("Anthropic adapter", () => {
  ...
  it("dispatches a tool_use block and returns tool_result", async () => {
    ...
    const result = await unwind.handleToolUse(
      runId,
      0,
      toolUseBlock,
      [readFile, sendEmail]
  ...
  it("throws for unknown tool name", async () => {
    ...
    await expect(
      unwind.handleToolUse(runId, 0, toolUseBlock, [readFile])
  ...
  it("supports a multi-step tool loop (two different tools)", async () => {
    ...
    const step0Result = await unwind.handleToolUse(
      runId,
      0,
      {
        type: "tool_use",
    ...
    const step1Result = await unwind.handleToolUse(
      runId,
      1,
      {
        type: "tool_use",
