...
describe("LangGraph adapter", () => {
    ...
    it("converts multiple tools preserving all args", () => {
        const lgTools = toLangGraphTools([readFile, createUser], async () => ({}));
    ...
    it("supports dispatching multiple different tools in sequence", async () => {
        const runId = unwind.startRun("test-agent");
        let step = 0;
        const lgTools = toLangGraphTools([readFile, createUser], async (tool, args) => unwind.dispatch(runId, step++, tool, args));
...
//# sourceMappingURL=langgraph.test.js.map