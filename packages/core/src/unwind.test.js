...
describe("Unwind – tool wrapping, dispatch & middleware", () => {
    ...
    it("throws when dispatching destructive tool without approval gate", async () => {
        ...
        const runId = unwind.startRun("test-agent");
        await expect(unwind.dispatch(runId, 0, deleteTool, { table: "users" })).rejects.toThrow(/approval gate/i);
        const run = unwind.getRun(runId);
        const failEvent = run.events.find((e) => e.type === "ToolCallFailed");
    ...
    it("logs approval events for destructive tool with gate", async () => {
        ...
        const approvalReq = run.events[0];
        expect(approvalReq.toolName).toBe("dropTable");
        const approvalRec = run.events[1];
    ...
    it("throws and logs failure when approval is denied", async () => {
        ...
        const runId = unwind.startRun("test-agent");
        await expect(unwind.dispatch(runId, 0, deleteTool, { table: "users" })).rejects.toThrow(/denied/i);
        const run = unwind.getRun(runId);
        const types = run.events.map((e) => e.type);
        expect(types).toEqual([
            "ApprovalRequested",
    ...
    it("logs ToolCallFailed on execution error", async () => {
        ...
        const runId = unwind.startRun("test-agent");
        await expect(unwind.dispatch(runId, 0, failTool, {})).rejects.toThrow("boom");
        const run = unwind.getRun(runId);
        const failEvent = run.events.find((e) => e.type === "ToolCallFailed");
        expect(failEvent).toBeDefined();
...
//# sourceMappingURL=unwind.test.js.map