...
describe("Unwind – compensation runner", () => {
  ...
  function makeIdempotentTool(name: string) {
    return unwind.tool({
      name,
  ...
  function makeReversibleTool(
    name: string,
    compensateImpl?: (args: any, result: any) => Promise<unknown>
  ) {
    return unwind.tool({
      name,
  ...
  function makeAppendOnlyTool(name: string) {
    return unwind.tool({
      name,
  ...
  function makeDestructiveTool(name: string) {
    return unwind.tool({
      name,
